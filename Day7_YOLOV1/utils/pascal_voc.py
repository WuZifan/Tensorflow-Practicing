...
class pascal_voc(object):
    ...
    def get(self):   #获取到batch_size大小的图片和对应的label
        ...
        labels = np.zeros(
            (self.batch_size, self.cell_size, self.cell_size, 25)) # 7*7*[20+(4+1)]
        count = 0
    ...
    def prepare(self):
        ...
        if self.flipped:
            ...
            '''
            gt_labels_cp是一个list，里面每一个元素都是一个字典，一个字典代表一个图片
            dict{
                imgname:名称
                label：【7*7*25】
                flipped:false
    ...
    def load_labels(self):    #加载标签文件，获取标签
        ...
        print('length',len(self.image_index))

        # index类似于图片名字
        # label
        # num表示有几个物体
        for index in self.image_index[:1]: #得到train或者test文件中所有指向图片的index,
            # 读取annotation文件, return label and len(objs)
            ...
            gt_labels.append({'imname': imname,   #图片的路径
                              'label': label, # 这里的label应该就是正常的gt坐标
                              'flipped': False})
