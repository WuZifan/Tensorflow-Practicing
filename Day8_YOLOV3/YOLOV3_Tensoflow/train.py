...
# tf.data pipeline
##################
# Selecting `feedable iterator` to switch between
...
'''
上面一大堆是准备训练数据和定义一些路径之类的参数的，一会再看
下面开始做网络的定义了
'''
...
'''
这个需要看一下，究竟是在干什么？在每个需要更新的ops后面都加BN层？
'''
...
with tf.Session() as sess:
    ...
    for epoch in range(args.total_epoches):
        for i in range(args.train_batch_num):
            ...
            #{is_training: True, handle_flag: train_handle_value}
