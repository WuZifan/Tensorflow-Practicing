...
用来计算RPN的结果的
返回的是：
    rpn_class结果：判断这个框是不是合理，以及是不是物体
    rpn_reg结果：这个框的坐标回归值
'''
def calc_rpn(C, img_data, width, height, resized_width, resized_height, img_length_calc_function):
    ...
    for anchor_size_idx in range(len(anchor_sizes)):
        for anchor_ratio_idx in range(n_anchratios):
            '''
            1、前面两个循环，是对每一种anchor
            2、这里的anchors都是在原图上的定义
            '''
            ...
            for ix in range(output_width):
                ...
                for jy in range(output_height):
                    '''
                    这两个循环，是对feature-map上的每一个位置
                    拿到每个anchors在原图上的上下y的坐标
                    '''
                    ...
                    '''
                    我已经得到了，某个特定的anchors_size和anchors_ratio,
                                在feature-map某个特定点，回归到原图上的坐标
                    下面是遍历，原始图像上，每一个gt，和这个anchors的IoU
                    '''
                    for bbox_num in range(num_bboxes):
                        ...
                        '''
                        1、看看有没有哪个gt和这个anchors非常符合
                                best_iou_for_bbox 这个最开始是个0啊
                           反正选个最大的呗…
                        2、但结合下面，会发现，你和bg的IOU最大，我们是不认的
                        3、没有和任何一个pos，或者neural配对成功的，就按照anchors的默认，neg标记。
                        '''
                        ...
                        if curr_iou > best_iou_for_bbox[bbox_num] or curr_iou > C.rpn_max_overlap:
                            ...
                            # th = log(h / ha)
                            '''
                            回归的目标是：让预测框相对于anchors的偏移，
                                         尽可能的和gt相对于anchor的偏移靠近
                            这里要的是label，因此计算的是gt相对于anchor的偏移
                            '''
                    ...
                    '''
                    经过上面那个循环，我们知道了每个anchors的类别。是pos，neg，还是neutral
                    不过话说neutral没用到啊…
                    '''
    ...
    '''
    要确保image上每个gt，至少有一个pos的anchors和它对应。
    '''
    ...
    '''
        这里相当于把过多的postive和negitive都设置为neural这一类
        限制每张原始图片的anchors中，差不多256/2个positive和256/2个negtive
    '''
